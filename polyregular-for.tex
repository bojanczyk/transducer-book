...
\issue{The examples use more than this syntax: \texttt{q = not q} in Example~\ref{ex:parity-for-transducer}, and finitely-valued variables together with \texttt{output(w[y])} in Figure~\ref{fig:map-reverse-for-transducer}. All three are syntactic sugar, but the text should say so.}
...
def isEmpty(w):
  ...
  if not q:
    output('not empty')
\end{lstlisting}
\issue{The test is inverted: Boolean variables start false, so \texttt{q} is false exactly when the input is empty, and this program prints ``not empty'' only on the empty input.}
