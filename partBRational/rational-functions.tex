...
\exer{\label{exer:no-reverse-reversible} Show that in the list of atomic functions from \cref{thm:rational-primes}, the reversal of a reversible Mealy machine is not needed.
}{
    Suppose that we want to implement a right-to-left  reversible Mealy machine with states $Q$. We first run normal left-to-right reversible Mealy machine to label each position  with the state transformation of the prefix leading up to that position.  Then we run a right-to-left flip-flop machine to label each position with the state transformation of the entire input string. Then, using cancellation in the group of state permutations, we get get for each position the state transformation of the suffix after that position. Using the state transformation on the suffixes, we can recover the output of the Mealy machine that we wanted to implement.
}
