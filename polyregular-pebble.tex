...
\flag{The definition of what a pebble sees is crucial for the finite-state argument, but the text does not spell out explicitly how the information about "pebbles in the same place" is encoded in bounded memory across updates. The stack discipline is mentioned informally, but the state-space bound is not stated as a theorem in this section.}
\flag{The fact that a pebble transducer with stack discipline remains finite-state is not a corollary of the definition alone; it is a nontrivial theorem that depends on the top-of-stack restriction. Without stating it formally, the transition relation looks as if it were just a bounded finite control but it hides a strong invariant about which pebbles may be modified and which positions remain accessible.}
