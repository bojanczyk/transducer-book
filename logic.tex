...
If $f$ is this rational function, then there is a one-to-one correspondence between $\tau(w)$ and positions in $f(w)$. This correspondence and the structure of order and labels can be defined in \mso, and thus the output of the original \mso transduction can be obtained by a new \mso transduction that uses type $n$.
    ...
    \end{enumerate}
\issue{Scanning for the first $y$ with $[x..y] \in L_\leq$ does not find the successor: every later output position also satisfies $\varphi_\leq(x,y)$. The set $\Phi$ should contain the (\mso-definable) successor formula of the output order, and step 4 should use its language.}
...
We now show that the $k$-types are compatible with operations on strings, which will allow us to define an automaton structure on them, and that the resulting automaton is aperiodic. (In the lemma below, a sequence is called aperiodic if it eventually reaches a stable value, i.e.~from some point on all elements in the sequence are the same.)
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item \textbf{Refinement.} Choose a  triple  in $ \fotype_{k+1}(w)$ and use the congruence property for $k$ to obtain  $\fotype_{k}(w)$.  The answer does not depend on the choice of triple.
\issue{The case $w = \varepsilon$, where $\fotype_{k+1}(w)$ is empty and there is no triple to choose, needs a word.}
...
\begin{proof}
    ...
    \end{align*}
    with $i > 0$,
    then we have sufficient information to compute the string produced by the relabelling on the $i$-th input position. 
\issue{The gap $i=0$, and the empty input, are not covered by the construction.}
\end{proof}
