...
| `PartC/CombTerms.lean` | Definition `def:regular-terms`: the syntax `Transducers.RegTerm`, its semantics `Transducers.RegTerm.eval`, and the auxiliary functions `Transducers.prefixProd` and `Transducers.splitList` of the two less standard atomic terms |
