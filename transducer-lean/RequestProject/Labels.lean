...
/-- **Exercise `exer:rational-injectivity-decidable`** (injectivity is decidable): `Transducers.Exercises.rationalFun_injectivity_decidable`, proved outright.  It used to take two hypotheses, `Transducers.EffectiveWeightedEvalEq` and `Transducers.Exercises.EffectiveRationalSection`; both are now theorems, the first of `RequestProject/PartB/WCodePrimrec.lean`, the second of `RequestProject/Exercises/RatInjectiveDec.lean` (from the construction of `RequestProject/Exercises/RatSection.lean` and `RequestProject/Exercises/RatSectionPrimrec.lean`). -/
