/-
Part B: Rational relations, rational functions, weighted automata and the
machine independent characterisations, from *Transducers* (M. Bojańczyk).

This file collects the contents of the directory `RequestProject/PartB`.  The numbered results of
Sections *Rational relations* to *Rational functions* are stated in
`RequestProject/PartB/RationalStatements.lean` and those of Sections *Rational relations and
weighted automata* to *Machine independent characterisations* in
`RequestProject/PartB/WeightedStatements.lean`; the other files contain the constructions used in
their proofs. -/
import RequestProject.PartB.LabAut
import RequestProject.PartB.Atomize
import RequestProject.PartB.OutLang
import RequestProject.PartB.EpsElim
import RequestProject.PartB.RatComp
import RequestProject.PartB.RatCont
import RequestProject.PartB.HomComplement
import RequestProject.PartB.MealyChar
import RequestProject.PartB.Typing
import RequestProject.PartB.LenNormalForm
import RequestProject.PartB.SeqChar
import RequestProject.PartB.Lcp
import RequestProject.PartB.SubseqDef
import RequestProject.PartB.SubseqAlpha
import RequestProject.PartB.SubseqState
import RequestProject.PartB.SubseqBound
import RequestProject.PartB.SubseqChar
import RequestProject.PartB.WeightedZero
import RequestProject.PartB.RationalStatements
import RequestProject.PartB.WeightedStatements
