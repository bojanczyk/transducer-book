...
`SnakeAlph*.lean` files).  It also collects `RegBoundGap.lean`, which states the effective form of
the book's reduction of regular equivalence to weighted automata and proves that it would suffice
for the equivalence bound of Theorem `thm:decidable-equivalence-regular`; that bound is in fact
obtained by another route, in `RegEffBound.lean`, so the theorem is unconditional. -/
