...
-/
import RequestProject.PartC.CombMach
...
theorem concatMach_tail (l : List (List A.Elt)) (rest : List Sym8) :
    concatMach.runFrom ((catDom A).height) (ConcatMode.outer, k1 A)
      ...
      = commaBlocks A l.flatten
        ++ concatMach.runFrom ((catDom A).height) (ConcatMode.outer, k1 A) rest := by
  induction l with
  | nil => rw [joinSepTail_nil, List.nil_append, List.flatten_nil, commaBlocks_nil,
      List.nil_append]
  | cons li l ih =>
      rw [joinSepTail_cons, joinSep_cons, List.cons_append, Mach.runFrom_cons]
...
theorem concatMach_body (l : List (List A.Elt)) (rest : List Sym8) :
  ...
  | nil => simp
  | cons li l =>
      rw [joinSep_cons, List.append_assoc, concatMach_inner, concatMach_tail, List.flatten_cons,
