...
\begin{proof}
    ...
    By Theorem~\ref{thm:bimachines}, we can assume that $f$ is computed by a bimachine.  We first apply the function $w \mapsto w\#$. Next, we run the past and future automata on the string, and label the input string with the states of these automata. The state is stored in the position to its right, which is why we use the separator symbol $\#$ to store the state at the end of the string. The past and future automata are Mealy machines, with the future one being a right-to-left Mealy machine. Finally, we apply a homomorphism, which produces the desired output string. 
\end{proof}
