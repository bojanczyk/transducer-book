...
\exer{Give an example of a function that is continuous, letter-to-letter (i.e.~input length is the same as output length), and is not computed by a "Mealy machine".}{The reverse function from \cref{exer:reverse-continuous}.
}

    \exer{   Show that a composition of $n$ "Mealy machines", each one with at most $n$ states, might require a number of states that is exponential in $n$.}{
