...
\paragraph*{Lean.} 
In the current stage all theorems are proved in Lean. However, Lean  currently struggles with algorithms (which is surprising to me), and hence some natural algorithmic properties are not formalised and are left as assumptions (e.g. one can effectively represent rational numbers and their field operations, or undecidability of the Post Correspondence Problem).  Finally, some exercise solutions  are not formalised, especially those that talk about algorithms or use external results (e.g. Presburger arithmetic). I will try to fix these issues over time. Hopefully some of them will be resolved by improvements in Mathlib that I expect to see soon.
