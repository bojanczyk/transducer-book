...
\begin{theorem}[{Krohn-Rhodes Theorem\footnote
    {\label{footnote:krohn-rhodes}The theorem was originally proved in 
    \cite[p. 454]{krohnRhodes1965}. Let us remark on the differences between the original statement and the present text. In the original, semigroup terminology is used, while the present text uses the language of Mealy machines. The original also has an extra kind of product -- parallel composition -- instead of using only sequential composition as in the present text. Finally, the original statement says that the groups in the decomposition must have been present in the original machine, which is also true here but not stated explicitly. The proof in this book is inspired by a proof about \textsc{ltl} from \cite[p.35]{wilke1999}.   }}] 
