...
\begin{definition}\label{def:rational-recognisable-subsets}
    ...
    \end{align*}
    \end{itemize}
\flag{Not formalised. The development has no notion of rational or recognisable subset of an abstract monoid, and the identification of the rational subsets of $A^*\times B^*$ with the rational relations of Definition~\ref{def:rational-relation} is not proved.}
\end{definition}
...
\flag{\lean{rationalRel\_equivalence\_undecidable} takes the undecidability of the Post correspondence problem as an \emph{explicit hypothesis} (\lean{PCP.Solvable}, \lean{PartB/\allowbreak PCPRed.lean}); only the reduction below is formalised, and it is equivalence, not universality, that it shows undecidable. \lean{PCP.Solvable} also asks for a \emph{nonempty} $w$, as it must: the way the problem is stated in the proof below, $w=\varepsilon$ solves every instance.}
\end{theorem}
\begin{proof}
