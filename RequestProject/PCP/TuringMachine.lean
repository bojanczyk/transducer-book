...
-/
import RequestProject.PCP.SRtoMPCP
import RequestProject.PCP.StarTrick

namespace PCP

/-- The symbols used to write configurations of a Turing machine, together with the three
auxiliary symbols `#`, `∗` and `◇` of Sipser's construction and the marker `⊢` opening a
computation history. -/
inductive Sym where
