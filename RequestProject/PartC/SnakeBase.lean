...
-/
import RequestProject.PartC.SnakeWidth
  ...
  · push_neg at hA
    have hlast : ∃ prev q, passAt M w w.length = Sum.inl (prev, q) := by
      ...
      · exfalso
        ...
        | true =>
            apply h
            rw [PassLang, Set.mem_setOf_eq, ← passAt_length, hp]
            rfl
    obtain ⟨prev, q, hpq⟩ := hlast
    obtain ⟨hprev, hcfg⟩ := cfgAt_of_passAt M w w.length (le_refl _) hpq
    have hnotacc : passAccB M (passAt M w w.length) ≠ true := by
      intro hc
      exact h (by rw [PassLang, Set.mem_setOf_eq, ← passAt_length]; exact hc)
    rw [hpq] at hnotacc
