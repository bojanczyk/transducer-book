...
lemma selected_toI (w : List A) (x : T.Elt) : T.toI.selected w x ↔ T.selected w x := by
  rcases x with ⟨i, p⟩ | j <;> exact Iff.rfl

lemma ordRel_toI (w : List A) (x y : T.Elt) : T.toI.ordRel w x y ↔ T.ordRel w x y := by
  rcases x with ⟨i, p⟩ | j <;> rcases y with ⟨i', p'⟩ | j' <;> exact Iff.rfl

lemma labRel_toI (w : List A) (x : T.Elt) (b : B) : T.toI.labRel w x b ↔ T.labRel w x b := by
  rcases x with ⟨i, p⟩ | j <;> exact Iff.rfl

lemma outputs_toI {w : List A} {v : List B} (h : T.Outputs w v) : T.toI.Outputs w v := by
  obtain ⟨es, hnd, hmem, hord, hlen, hlab⟩ := h
  refine ⟨es, hnd, fun x => ?_, fun i j hi hj hij => ?_, hlen, fun i hi hi' => ?_⟩
  · rw [T.selected_toI w x]; exact hmem x
  · rw [T.ordRel_toI w _ _]; exact hord i j hi hj hij
  · rw [T.labRel_toI w _ _]; exact hlab i hi hi'

lemma proper_toI (h : T.Proper) : T.toI.Proper := by
  intro w
  ...
  · obtain ⟨b, hb, hu⟩ := hlab x ((T.selected_toI w x).1 hx)
    exact ⟨b, (T.labRel_toI w x b).2 hb, fun b' hb' => hu b' ((T.labRel_toI w x b').1 hb')⟩
  · exact (T.ordRel_toI w x x).2 (hrefl x ((T.selected_toI w x).1 hx))
  · exact hanti x y ((T.selected_toI w x).1 hx) ((T.selected_toI w y).1 hy)
      ((T.ordRel_toI w x y).1 hxy) ((T.ordRel_toI w y x).1 hyx)
  · exact (T.ordRel_toI w x z).2 (htrans x y z ((T.selected_toI w x).1 hx)
      ((T.selected_toI w y).1 hy) ((T.selected_toI w z).1 hz)
      ((T.ordRel_toI w x y).1 hxy) ((T.ordRel_toI w y z).1 hyz))
  · rcases htot x y ((T.selected_toI w x).1 hx) ((T.selected_toI w y).1 hy) with h' | h'
    · exact Or.inl ((T.ordRel_toI w x y).2 h')
    · exact Or.inr ((T.ordRel_toI w y x).2 h')

lemma allFO_toI (h : T.AllFO) : T.toI.AllFO :=
...
theorem exists_itrans_of_isFOTransduction {A B : Type} {f : List A → List B}
  ...
  exact ⟨T.toI, T.proper_toI hP, T.allFO_toI hFO, fun w => T.outputs_toI (hO w)⟩
