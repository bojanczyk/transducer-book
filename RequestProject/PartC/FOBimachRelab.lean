...
bimachine.
-/
import RequestProject.PartC.FORev
...
theorem isFORelabelling_of_isAperiodicBimachine {A B : Type} [Finite A] {f : List A → List B}
  ...
  choose χ hχfo hχfree hχsat using fun q : P =>
    (foDefinable_state_lang M.prefixStep hpre M.prefixInit q).exists_sentence
  choose ξ hξfo hξfree hξsat using fun s : S =>
    (foDefinable_rev_state_lang M.suffixStep hsuf M.suffixInit s).exists_sentence
