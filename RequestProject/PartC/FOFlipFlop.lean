...
In a flip-flop machine (Definition A.2.1) every letter either leaves the state
unchanged or resets it to a fixed state.  Hence the state reached after reading
a prefix of the input is the target of the *last* resetting letter of that
prefix, and the initial state if the prefix has no resetting letter.  This is
the combinatorial content of the fact that flip-flop machines are first-order
definable, used in Theorem C.4.11; the corresponding formulas are built in
...
lemma letterTrans_of_resetTo_none {M : Mealy A B Q} {a : A} (h : M.resetTo a = none) (q : Q) :
...
lemma letterTrans_of_resetTo_some {M : Mealy A B Q} (hM : M.FlipFlop) {a : A} {q₀ : Q}
    (h : M.resetTo a = some q₀) (q : Q) : M.letterTrans a q = q₀ := by
  by_cases hid : M.letterTrans a = id
...
lemma trans_of_all_resetTo_none {M : Mealy A B Q} :
    ∀ (u : List A), (∀ a ∈ u, M.resetTo a = none) → ∀ q : Q, M.trans u q = q := by
  intro u
  induction u with
  | nil => intro _ q; rfl
  | cons a u ih =>
...
lemma trans_take_of_last_reset {M : Mealy A B Q} (hM : M.FlipFlop) {w : List A} {p j : ℕ}
    {a : A} {q₀ : Q} (hj : j < p) (hwj : w[j]? = some a) (hreset : M.resetTo a = some q₀)
    (hlast : ∀ z a', j < z → z < p → w[z]? = some a' → M.resetTo a' = none) :
...
lemma trans_take_of_no_reset {M : Mealy A B Q} {w : List A} {p : ℕ}
    (hnone : ∀ z a', z < p → w[z]? = some a' → M.resetTo a' = none) :
...
theorem trans_take_eq_iff {M : Mealy A B Q} (hM : M.FlipFlop) (w : List A) (p : ℕ) (q : Q) :
    M.trans (w.take p) M.init = q ↔
      ((∃ j < p, ∃ a, w[j]? = some a ∧ M.resetTo a = some q ∧
          ∀ z a', j < z → z < p → w[z]? = some a' → M.resetTo a' = none)
        ∨ (q = M.init ∧ ∀ z a', z < p → w[z]? = some a' → M.resetTo a' = none)) := by
  classical
  constructor
  · intro htr
    by_cases hres : ∃ j < p, ∃ a, w[j]? = some a ∧ (M.resetTo a).isSome
    · -- there is a resetting letter before `p`; take the last one
      obtain ⟨j₀, hj₀p, ⟨a₀, hwj₀, hsome₀⟩, hmax⟩ :=
        exists_last_lt (fun j => ∃ a, w[j]? = some a ∧ (M.resetTo a).isSome) p
          (by obtain ⟨j, hjp, hj⟩ := hres; exact ⟨j, hjp, hj⟩)
      obtain ⟨q₀, hq₀⟩ := Option.isSome_iff_exists.mp hsome₀
      have hlast : ∀ z a', j₀ < z → z < p → w[z]? = some a' → M.resetTo a' = none := by
        intro z a' hz1 hz2 hz3
        have hnot := hmax z hz1 hz2
        cases hr : M.resetTo a' with
        | none => rfl
    ...
    · push_neg at hres
      have hnone : ∀ z a', z < p → w[z]? = some a' → M.resetTo a' = none := by
        intro z a' hz hwz
