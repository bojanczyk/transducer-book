...
lemma freeFO_lt_maxVar : ∀ (φ : MSO A) {i : ℕ}, i ∈ φ.freeFO → i < φ.maxVar := by
  intro φ
  induction φ with
...
lemma freeSO_lt_maxVar : ∀ (φ : MSO A) {i : ℕ}, i ∈ φ.freeSO → i < φ.maxVar := by
  intro φ
  induction φ with
...
lemma maxVar_rename_add (d : ℕ) :
    ∀ φ : MSO A, (rename (· + d) φ).maxVar = φ.maxVar + d := by
  intro φ
  induction φ with
...
lemma maxVar_shiftUp (d : ℕ) (φ : MSO A) : (shiftUp d φ).maxVar = φ.maxVar + d :=
...
def atv (v : ℕ) (φ : MSO A) : MSO A :=
  bindSO (v + 1) (shiftUp (v + 1) φ).maxVar
    (bindRange v (v + 1) (shiftUp (v + 1) φ).maxVar (shiftUp (v + 1) φ))
...
lemma sat_atv (w : List A) (v : ℕ) (φ : MSO A) (fo : ℕ → ℕ) (so : ℕ → Set ℕ)
  ...
  · have h1 : i < φ.maxVar := freeFO_lt_maxVar φ hi
    have h2 : φ.maxVar ≤ k := by rw [hk, hψ, maxVar_shiftUp]; omega
    rw [if_pos (by omega)]
  · have h1 : j < φ.maxVar := freeSO_lt_maxVar φ hj
    have h2 : φ.maxVar ≤ k := by rw [hk, hψ, maxVar_shiftUp]; omega
    rw [if_pos (by omega)]
...
def atv2 (v₀ v₁ : ℕ) (φ : MSO A) : MSO A :=
  let d := max v₀ v₁ + 1
  bindSO d (shiftUp d φ).maxVar
    (bindTo v₀ d (bindRange v₁ (d + 1) (shiftUp d φ).maxVar (shiftUp d φ)))
...
lemma sat_atv2 (w : List A) (v₀ v₁ : ℕ) (φ : MSO A) (fo : ℕ → ℕ) (so : ℕ → Set ℕ)
  ...
  rw [sat_bindRange w (show v₁ < d + 1 by omega) k ψ _ _ (by rw [hup]; exact h1), hup,
    sat_shiftUp]
  refine sat_congr w φ _ _ _ _ (fun i hi => ?_) (fun j hj => ?_)
  · have hlt : i < φ.maxVar := freeFO_lt_maxVar φ hi
    have hle : φ.maxVar ≤ k := by rw [hk, hψ, maxVar_shiftUp]; omega
    by_cases h : i = 0
    · subst h
      rw [if_neg (by omega), Nat.zero_add, Function.update_self, if_pos rfl]
    · rw [if_pos (by omega), if_neg h]
  · have hlt : j < φ.maxVar := freeSO_lt_maxVar φ hj
    have hle : φ.maxVar ≤ k := by rw [hk, hψ, maxVar_shiftUp]; omega
    rw [if_pos (by omega)]
