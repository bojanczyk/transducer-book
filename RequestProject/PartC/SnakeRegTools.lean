...
private lemma isRegularFun_casesOn_list (d : List A → D) (g : D → List A → List B)
  ...
  | cons e l ih =>
      ...
      intro u
      by_cases he : d u = e
      · rw [if_pos (show u ∈ {v : List A | d v = e} from he),
          if_pos (show d u ∈ e :: l from by simp [he]), he]
      · have hbeta : (fun v => if d v ∈ l then g (d v) v else ([] : List B)) u
            = if d u ∈ l then g (d u) u else ([] : List B) := rfl
        rw [if_neg (show u ∉ {v : List A | d v = e} from he), hbeta]
        by_cases hl : d u ∈ l
