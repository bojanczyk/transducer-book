/-
Claim C.4.14: compositionality of first-order logic over strings, which is the
left-to-right implication of Lemma C.4.13 in *Transducers* (M. Bojańczyk).
...
-/
import RequestProject.PartC.FOSeg
...
`k`-types (Lemma C.4.15, congruence). -/
...
/-- Level `k + 1` implies level `k` (refinement, Lemma C.4.15). -/
...
/-- The quantifier step of Claim C.4.14: a new marked position of `w` can be
matched by a marked position of `v`, at the cost of decreasing the level by
one. -/
...
/-- **Claim C.4.14.**  Whether a first-order formula of quantifier rank at most
`k` holds in a string with marked positions depends only on the level-`k`
information about the marked positions. -/
