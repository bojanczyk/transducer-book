...
-/
import RequestProject.PartC.FOSubstRel
import RequestProject.PartC.FOFlipFlop
  ...
  · intro c
    ...
    have hχ : ∀ b : B, ∃ χ : MSO A, χ.IsFO ∧ χ.freeFO = ∅ ∧
      ...
      have hφsat : ∀ (w : List A) (fo : ℕ → ℕ) (so : ℕ → Set ℕ),
        ...
        rw [this]
        simp
      refine ⟨MSO.shiftUp ψ.maxVar (MSO.closeSelf φ₀),
        MSO.isFO_shiftUp _ (MSO.isFO_closeSelf hφ₀fo),
        MSO.freeFO_shiftUp_eq_empty _ (MSO.freeFO_closeSelf φ₀), ?_, ?_⟩
      · intro i hi hmem
        have h1 : i < ψ.maxVar := MSO.mem_foVars_lt_maxVar ψ hi
        have h2 : ψ.maxVar ≤ i := MSO.le_of_mem_foVars_shiftUp _ _ hmem
        omega
      · intro w fo so
...
noncomputable def resetAtF [Fintype A] (M : Mealy A B Q) (v : ℕ) : MSO A :=
  bigOr ((Finset.univ : Finset A).toList.map
    (fun a => if (M.resetTo a).isSome then MSO.lab a v else MSO.ff))

lemma isFO_resetAtF [Fintype A] (M : Mealy A B Q) (v : ℕ) : (resetAtF M v).IsFO := by
  refine isFO_bigOr _ ?_
  intro φ hφ
  obtain ⟨a, -, rfl⟩ := List.mem_map.mp hφ
  by_cases h : (M.resetTo a).isSome <;> simp [h, MSO.ff, MSO.tt, MSO.IsFO]

lemma sat_resetAtF [Fintype A] (M : Mealy A B Q) (v : ℕ) (w : List A) (fo : ℕ → ℕ)
    (so : ℕ → Set ℕ) :
    Sat w fo so (resetAtF M v) ↔ ∃ a, w[fo v]? = some a ∧ (M.resetTo a).isSome := by
  rw [resetAtF, sat_bigOr]
  constructor
  · rintro ⟨φ, hφ, hsat⟩
    obtain ⟨a, -, rfl⟩ := List.mem_map.mp hφ
    by_cases h : (M.resetTo a).isSome
    · rw [if_pos h] at hsat
      exact ⟨a, hsat, h⟩
    · rw [if_neg h] at hsat
...
noncomputable def stateF [Fintype A] (M : Mealy A B Q) (q : Q) : MSO A :=
  MSO.or
    (bigOr ((Finset.univ : Finset A).toList.map
      (fun a => if M.resetTo a = some q then
...
lemma sat_stateF [Fintype A] (M : Mealy A B Q) (hM : M.FlipFlop) (q : Q) (w : List A)
  ...
  · rintro (hleft | hright)
    · -- the last resetting letter
      ...
      obtain ⟨a, -, rfl⟩ := List.mem_map.mp hφ
      by_cases hres : M.resetTo a = some q
...
lemma foDefMealy_flipFlop [Fintype A] [Fintype Q] (M : Mealy A B Q)
  ...
  refine ⟨fun w => M.eval_length w, M.prefixDetermined, fun b => ⟨outF M b, isFO_outF M b, ?_⟩⟩
  intro w fo so
  ...
  · rintro ⟨p, hp, hlast, hbig⟩
    ...
    · rw [if_pos hb] at hsat
      ...
      rw [M.getLast?_eval w p hpe qa.2 hlab', hst, hb]
  ...
  · intro hb
    ...
    have hout : (M.step q a).2 = b := by
      have := M.getLast?_eval w p hpe a ha
      rw [hb] at this
