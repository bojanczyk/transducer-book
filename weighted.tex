...
In this section, we describe the connections of rational relations (including their functional fragment) with another automaton model, called weighted automata. Using this connection and a certain decidability result for weighted automata, we will show that equivalence of rational functions is decidable. This  connection will also be used in the future, to decide equivalence for a more powerful transducer model, namely the regular functions. 
...
\begin{definition}
    [Semiring] A semiring is a set $\semiring$ equipped with two binary operations $+$ and $\times$, and two distinguished elements $0$ and $1$, subject to  the axioms in Figure~\ref{fig:semiring-axioms}.
\flag{Formalised as Mathlib's \lean{Semiring}. The axiom table of Figure~\ref{fig:semiring-axioms} is incomplete: it lists $1 \times x = x$ but not $x \times 1 = x$, and the line $0 \times x = 0 = 0 \times x$ repeats one axiom where the first equation should be $x \times 0 = 0$.}
\end{definition}
...
\begin{definition}[Weighted automaton]
    ...
    \end{enumerate}
\flag{\lean{LabAut.wEval} (\lean{PartB/\allowbreak LabAut.lean}) sums over the \emph{lists of transitions} of the accepting runs. A nonempty run is determined by its transition list, but the empty run is not: if $k>1$ states are both initial and final, the reading here gives $k$ accepting runs over $\varepsilon$, whereas the formalisation counts one. The normalisation \lean{initCopy} (\lean{PartB/\allowbreak WeightedNF.lean}) removes the discrepancy.}
\end{definition}
...
\begin{theorem}\label{thm:equivalence-weighted-automata}
    Given two weighted automata over the field of rationals, it is decidable whether they compute the same function.    
\flag{\lean{weighted\_equivalence\_decidable} is proved \emph{from an explicit effectivity hypothesis} \lean{EffectiveWeightedEvalEq} (\lean{PartB/\allowbreak Effective.lean}): that the values of two coded weighted automata over $\Rat$ on a given string can be compared by a computable procedure. Everything else, including the effective form of the Sch\"utzenberger bound, is proved. The hypothesis is needed only because Mathlib's \lean{Computable} API has no arithmetic on $\Int$ or $\Rat$; the unconditional statement is kept, commented out, in \lean{PartB/\allowbreak WeightedStatements.lean}.}
\end{theorem}
The above result is well-known, but in the interest of completeness, we give a proof of it in the next section.  We have stated it for the field of rationals, which is the field that we will use in our applications to string-to-string transducers. Nevertheless, the same proof will work  for any other field, as long as field elements can be represented in a finite way and field operations can be computed. Let us now show how to use the above result to decide equivalence of rational functions.
\begin{theorem}\label{thm:equivalence-rational-functions}
    The equivalence problem $f=g$ is decidable for rational functions.
\flag{\lean{rationalFun\_equivalence\_decidable} inherits the effectivity hypothesis of Theorem~\ref{thm:equivalence-weighted-automata} and nothing else; the reduction below, including the encoding $\iota$, is formalised in full (\lean{PartB/\allowbreak RatEqDec.lean}). Decidability is stated for codes, under the promise that the coded relation is a total function \emph{on the strings over the alphabet of the code}.}
\end{theorem}
\begin{proof}
    ...
    \end{lemma}
    \begin{proof}
        We use a product construction. To make this construction correct, we want to ensure that there is a one-to-one correspondence between accepting runs of the product automaton and runs of the second weighted automaton. This will be achieved by making the first automaton (for $f$) unambiguous, and appropriately aligning the two runs in the product constructions.
        ...
        The states of the new weighted automaton in the conclusion of the theorem are pairs (state of $f$, state of $h$), plus two extra states $\vdash$ and  $\dashv$. The initial states are $\vdash$ and every pair $(p,q)$ that  is initial on both coordinates, in the corresponding automata. The state $\dashv$ is the unique final state. Transitions between states that are pairs are of the form
        \begin{align*}
        (p,q) \xrightarrow {a/s} (p',q')
        \end{align*}
        where $a$ is an input letter and $s \in \semiring$ is the sum of weights of runs in the weighted automaton that have the following properties: the source state is $q$, the target state is $q'$, the input string is $a$, and the letter is consumed in the last transition. To reach the unique final state from pair states, we have transitions of the form
        \begin{align*}
...
\begin{theorem}\label{thm:characterisation-rational-functions-weighted-automata}
    A string-to-string function $f : A^* \to B^*$ is rational if and only if weighted automata are closed under pre-composition with $f$.
\end{theorem}
\begin{proof}
    The left-to-right implication was shown in Lemma~\ref{lem:closure-weighted-automata-precomposition}. For the right-to-left implication, consider the semiring of regular languages, and the  function 
    \begin{align*}
...
\begin{theorem}\label{thm:zeroness-weighted-automata}
...
\flag{\lean{weighted\_zeroness\_decidable} is obtained as the special case of Theorem~\ref{thm:equivalence-weighted-automata} in which the second automaton is empty, so it carries the same hypothesis \lean{EffectiveWeightedEvalEq}. Only $\Rat$ is formalised; the footnote's ``any computable field'' is not. The saturation proof below is formalised in \lean{PartB/\allowbreak WeightedZero.lean} as Sch\"utzenberger's criterion with the explicit length bound $d_1+d_2$.}
\end{theorem}
